...
async function showOffice2Action(actionId,source="touch"){if(!state.display){toast("⚠ DISPLAY 연결을 확인하세요");return}const payload={id:crypto.randomUUID(),type:"AI_OFFICE_ACTION",actionId,source,payload:{agent:actionId.split(":")[0],action:actionId.slice(actionId.indexOf(":")+1)},sentAt:new Date().toISOString(),version:"1.9.8"};const verify=$("actionSendVerify");if(verify)verify.textContent=`송신 ACTION · ${actionId}`;state.pending.set(payload.id,setTimeout(()=>{state.pending.delete(payload.id);toast("⚠ 화면 연결을 확인하세요")},AI_OFFICE_CONFIG.commandTimeoutMs));toast(`${actionId} 전송 중…`);await sendBroadcast(state.channel,"command",payload)}
