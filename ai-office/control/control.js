...
const office2Actions=[["aria:today","오늘 업무"],["aria:week","이번 주"],["aria:month","이번 달"],["aria:schedule","일정·D-DAY"],["aria:task","TASK"],["aria:project","PROJECT"],["aria:meeting","회의 준비"],["gen:news","뉴스 브리핑"],["gen:article","기사 준비"],["gen:library","자료 보기"],["gen:media","미디어"]];
async function showOffice2Action(actionId,source="touch"){if(!state.display){toast("⚠ DISPLAY 연결을 확인하세요");return}const payload={id:crypto.randomUUID(),type:"AI_OFFICE_ACTION",actionId,source,payload:{agent:actionId.split(":")[0],action:actionId.split(":")[1]},sentAt:new Date().toISOString(),version:"1.5.0"};state.pending.set(payload.id,setTimeout(()=>{state.pending.delete(payload.id);toast("⚠ 화면 연결을 확인하세요")},AI_OFFICE_CONFIG.commandTimeoutMs));toast(`${actionId.split(":")[1].toUpperCase()} 전송 중…`);await sendBroadcast(state.channel,"command",payload)}
