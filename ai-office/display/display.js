import{db,$,clean,isSafeHttpUrl,youtubeId,loadCatalog,createPresentationChannel,sendBroadcast,requireSession}from"../shared/core.js";
const state={contents:[],channel:null,code:"",current:""};const views=["homeView","imageView","videoView","webView"];
function active(id){views.forEach(view=>$(view).classList.toggle("active",view===id))}
function theme(value){$("stage").className=`stage theme-${clean(value||"DARK_GOLD")}`}
function stopMedia(){$("playerWrap").innerHTML="";$("webFrame").src="about:blank"}
async function ack(commandId,ok,message=""){await sendBroadcast(state.channel,"ack",{commandId,ok,message,displayedAt:new Date().toISOString()})}
async function show(item,commandId){if(!item){ack(commandId,false,"자료를 찾을 수 없습니다.");return}$("loading").hidden=false;stopMedia();theme(item.background_theme);views.forEach(id=>$(id).style.transition=item.transition==="NONE"?"none":"");try{
 if(item.content_type==="HOME"){active("homeView")}
 else if(item.content_type==="IMAGE"){if(!isSafeHttpUrl(item.source_url)&&!item.source_url.startsWith("../")&&!item.source_url.startsWith("/"))throw new Error("허용되지 않은 이미지 주소");await new Promise((resolve,reject)=>{const img=$("contentImage");img.onload=resolve;img.onerror=reject;img.src=item.source_url});active("imageView")}
 else if(item.content_type==="YOUTUBE"){const id=youtubeId(item.source_url);if(!id)throw new Error("YouTube 주소 오류");const frame=document.createElement("iframe");frame.allow="autoplay; encrypted-media; picture-in-picture";frame.allowFullscreen=true;frame.src=`https://www.youtube-nocookie.com/embed/${id}?autoplay=1&rel=0&playsinline=1`;frame.title=item.title;$("playerWrap").append(frame);active("videoView")}
 else if(item.content_type==="VIDEO"){if(!isSafeHttpUrl(item.source_url))throw new Error("영상 주소 오류");const video=document.createElement("video");video.src=item.source_url;video.controls=true;video.autoplay=true;video.playsInline=true;$("playerWrap").append(video);active("videoView");video.play().catch(()=>{})}
 else if(item.content_type==="WEB"){if(!isSafeHttpUrl(item.source_url))throw new Error("웹 주소 오류");$("webHelp").hidden=item.web_embed_allowed!==false;$("webFrame").hidden=item.web_embed_allowed===false;$("webLink").href=item.source_url;if(item.web_embed_allowed===false)active("webView");else{$("webFrame").src=item.source_url;active("webView")}}
