...
(function(){
  ...
  function evidenceLine(name){
    ...
    const parts=[];if(e.match)parts.push(`DB ${e.match}`);if(e.joined)parts.push(`등록 ${e.joined}`);if(e.feeDueDay)parts.push(`납부일 ${e.feeDueDay}일`);if(e.monthlyFee)parts.push(`기존 원비 ${Number(e.monthlyFee).toLocaleString('ko-KR')}원`);if(e.review)parts.push(`⚠ ${e.review}`);
    return parts.join(' · ');
  }
  function rowHtml(name,db){
    const r=ensureRecord(db,name),ev=evidenceLine(name);
