...
(function(){
  ...
  window.KMT_TUITION_PAYER_ALIAS_SEED=Object.freeze(seed.map(x=>Object.freeze({...x,students:Object.freeze([...x.students])})));
  let current=[];
  try{current=JSON.parse(localStorage.getItem(KEY)||'[]');if(!Array.isArray(current))current=[];}catch{current=[];}
  const same=(a,b)=>a.payer===b.payer&&Array.isArray(a.students)&&Array.isArray(b.students)&&a.students.join('|')===b.students.join('|');
  let changed=false;
  seed.forEach(s=>{
    if(!current.some(a=>same(a,s))){current.push({...s,updatedAt:new Date().toISOString()});changed=true;}
  });
