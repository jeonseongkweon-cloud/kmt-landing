...
function expose(ok,state){
  window.KMTTuitionMaster={
    ok,state,
    saveDue,
    saveMonth,
    saveInline,
    reload:pullRemoteIntoLocal
  };
}
...
async function ensureHousehold(row, override={}){
  ...
  const {data,error}=await db.from('kmt_tuition_households').upsert({
    household_key:householdKey,
    display_name:displayName,
    students_key:studentsKey,
    due_day:Math.min(31,Math.max(1,dueDay)),
    updated_at:new Date().toISOString()
  },{onConflict:'household_key'}).select('id,household_key,students_key,due_day').single();
...
async function saveDue(payload={}){
  ...
  if(!h) return false;
  const oldDay=Number(h.due_day||0);
...
async function saveMonth(payload={}){
  ...
  if(!h) return false;
  const month=Number(payload.appliedMonth)+1;
  const originalMonth=Number(payload.originalMonth)+1;
  if(originalMonth!==month){
    await db.from('kmt_tuition_monthly_records').delete().eq('household_id',h.id).eq('year',YEAR).eq('month',originalMonth);
  }
  const entryText=payload.entryText ?? '';
  const amountText=payload.amountText ?? payload.amount ?? '';
  const record={
    household_id:h.id,year:YEAR,month,
    paid_on:payload.paidOn||entryToDate(entryText),
    amount:parseAmount(payload.amount),
    payment_method:payload.method||inferMethod(entryText)||null,
    status:payload.status||inferStatus(entryText),
    entry_text:entryText||null,
    amount_text:amountText||null,
    memo:payload.memo||null,
    source:'tuition_admin',updated_at:new Date().toISOString()
  };
...
async function saveInline(input){
  ...
  if(!row) return false;
  const parts=String(input.dataset.k||'').split('|');
  if(parts.length<3) return false;
  const field=parts.pop();
  const month=Number(parts.pop());
  const studentsKey=parts.join('|');
  ...
  return saveMonth({
    row,householdKey:row.dataset.householdKey,displayName:row.querySelector('.lg-name b')?.textContent?.trim(),studentsCsv:row.dataset.students,
    originalMonth:month,appliedMonth:month,
    entryText,amountText,amount:amountText,
    paidOn:entryToDate(entryText),method:inferMethod(entryText),status:inferStatus(entryText),memo:''
  });
}
...
function updateNote(){
  ...
  if(note) note.textContent='※ 수정값은 브라우저에 안전 백업되며, 관리자 로그인 상태에서는 Supabase 중앙 회비 DB에도 자동 저장됩니다.';
}
...
function observeRows(){
  ...
  const mo=new MutationObserver(updateNote);mo.observe(root,{childList:true,subtree:true});
}

document.addEventListener('change',e=>{
  const input=e.target.closest?.('#ledgerGridCard input[data-k]');
  if(!input) return;
  setTimeout(()=>saveInline(input),0);
},true);
...
document.addEventListener('submit',e=>{
  ...
  if(e.target.id==='ledgerMonthForm'){
    ...
    const applied=Number(document.getElementById('ledgerMonthApplied')?.value||0);
    const original=Number(document.getElementById('ledgerMonthOriginal')?.value||0);
    ...
    const amount=document.getElementById('ledgerMonthAmount')?.value?.trim()||'';
