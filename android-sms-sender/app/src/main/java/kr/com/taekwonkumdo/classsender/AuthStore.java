...
public final class AuthStore {
    ...
    public synchronized String token() throws Exception{String a=p.getString("access",null);if(a==null)return null;if(System.currentTimeMillis()<p.getLong("expires",0)-120000)return a;String r=p.getString("refresh",null);if(r==null){clear();return null;}try{JSONObject body=new JSONObject().put("refresh_token",r);JSONObject out=ApiClient.rawPost("/auth/v1/token?grant_type=refresh_token",body,null);a=out.getString("access_token");p.edit().putString("access",a).putString("refresh",out.optString("refresh_token",r)).putLong("expires",System.currentTimeMillis()+out.optLong("expires_in",3600)*1000).apply();return a;}catch(Exception e){String m=e.getMessage();if(m!=null&&(m.contains("refresh_token_not_found")||m.contains("refresh_token_already_used"))){clear();return null;}throw e;}}
