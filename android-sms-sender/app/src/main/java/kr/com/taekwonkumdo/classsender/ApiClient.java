...
public final class ApiClient {
    ...
    private Object rpcAny(String fn,JSONObject body)throws Exception{String raw=requestRaw("POST","/rest/v1/rpc/"+fn,body);String t=raw.trim();return t.startsWith("[")?new JSONArray(t):new JSONObject(t);}
    private static String requestRaw(String method,String path,JSONObject body)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(AppConfig.SUPABASE_URL+path).openConnection();c.setRequestMethod(method);c.setConnectTimeout(12000);c.setReadTimeout(15000);c.setRequestProperty("apikey",AppConfig.PUBLISHABLE_KEY);c.setRequestProperty("Content-Type","application/json");if(body!=null){c.setDoOutput(true);try(OutputStream o=c.getOutputStream()){o.write(body.toString().getBytes(StandardCharsets.UTF_8));}}int code=c.getResponseCode();InputStream in=code>=200&&code<300?c.getInputStream():c.getErrorStream();StringBuilder b=new StringBuilder();if(in!=null)try(BufferedReader r=new BufferedReader(new InputStreamReader(in,StandardCharsets.UTF_8))){String line;while((line=r.readLine())!=null)b.append(line);}if(code<200||code>=300)throw new IOException("서버 "+code+": "+b);return b.toString();}
}
