...
async function saveAttendance(student,status,period){
  ...
  const{data:rows,error:e1}=await db.from("attendance").select("id,session_id,checked_at").eq("attendance_date",localDate()).eq("student_id",student.id).order("checked_at",{ascending:false}).limit(1);if(e1)throw e1;const old=rows?.[0]||null;
  const payload={session_id:old?.session_id||session.id,student_id:student.id,attendance_date:localDate(),status,checked_at:new Date().toISOString(),checked_out_at:null,points_awarded:["present","late"].includes(status)?1:0};
...
async function bulkPresent(period){if(!await hasPermission("attendance"))throw new Error("출석 처리 권한이 없습니다.");const session=await getSession(period,{create:true,open:true}),roster=rosterFor(period);for(const s of roster){const{data:rows,error}=await db.from("attendance").select("id,session_id,checked_at").eq("attendance_date",localDate()).eq("student_id",s.id).order("checked_at",{ascending:false}).limit(1);if(error)throw error;const old=rows?.[0]||null,payload={session_id:old?.session_id||session.id,student_id:s.id,attendance_date:localDate(),status:"present",checked_at:new Date().toISOString(),checked_out_at:null,points_awarded:1};const r=old?await db.from("attendance").update(payload).eq("id",old.id):await db.from("attendance").insert(payload);if(r.error)throw r.error}return roster.length}
