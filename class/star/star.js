...
function renderCategories(){
  ...
  $("categoryBar").innerHTML=`<button class="category praise-primary ${primary.id===state.category?.id?"active":""}" data-id="${primary.id}">⭐ 칭찬별</button><details class="category-more"><summary>더보기 ▾</summary><div class="category-more-list">${details.map(c=>`<button class="category ${c.id===state.category?.id?"active":""}" data-id="${c.id}">${c.icon} ${escapeHtml(c.name)}</button>`).join("")}</div></details>`;
