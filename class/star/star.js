...
function matchStudentFromText(text,{allowGlobalExact=false}={}){
  ...
  for(const s of roster){
    ...
    const tokens=[raw,...raw.split(/별|스타|칭찬|출석|체크|왔어|왔어요|도착|지각|단정|인사|자세|발차기|효도|배려|인성|정리|도전|게임|미션|성공/).filter(Boolean)];
...
function startOneShotVoice(mode){
  ...
  state.voice.mode=mode;setVoiceFeedback(mode==="attendance"?"🎤 음성 출석 준비":mode==="star"?"⭐ 음성 STAR 준비":"🎙 계명아 듣기 준비",mode==="attendance"?"예: 김나라 출석 / 김강민 왔어":mode==="star"?"예: 김나라 별 / 김강민 인성별":"예: 김나라 출석 / 김강민 인성별");
...
function playIncomingStarFeedback(rows){
  ...
  rows.forEach((event,index)=>setTimeout(()=>{
    ...
    if(category)speakShort(`${student.name} ${categoryDisplayName(category)} 하나!`);else speakShort(`${student.name} 별 하나!`);
    if(category)showBurst(student,category,total,[]);
    $("saveStatus").textContent="다른 기기 STAR 수신 · 🔊 재생";
...
$("loginButton").onclick=login;$("logoutButton").onclick=async()=>{await db.auth.signOut();location.reload()};$("backButton").onclick=()=>{location.href="../attendance/"};$("praiseForm").onsubmit=savePraise;$("championButton").onclick=showChampions;document.querySelectorAll("[data-close]").forEach(b=>b.onclick=()=>$(b.dataset.close).close());$("starBurst").onclick=()=>$("starBurst").hidden=true;db.auth.onAuthStateChange((_e,s)=>{if(s&&$("app").hidden)setTimeout(boot,0)});boot();