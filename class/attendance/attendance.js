...
function playAttendanceSuccessSound(){try{const C=window.AudioContext||window.webkitAudioContext;if(!C)return;const ctx=window.__kmtAttendanceAudio||(window.__kmtAttendanceAudio=new C());if(ctx.state==="suspended")ctx.resume();const now=ctx.currentTime;[[660,0],[990,.1]].forEach(([f,d],i)=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type=i?"triangle":"sine";o.frequency.setValueAtTime(f,now+d);g.gain.setValueAtTime(.0001,now+d);g.gain.exponentialRampToValueAtTime(i===1?.12:.1,now+d+.015);g.gain.exponentialRampToValueAtTime(.0001,now+d+.25);o.connect(g).connect(ctx.destination);o.start(now+d);o.stop(now+d+.27)})}catch(e){console.warn("[ATTENDANCE SOUND]",e)}}
function speakAttendance(text){if(!("speechSynthesis"in window))return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang="ko-KR";u.rate=1.14;u.pitch=1.15;u.volume=1;const voices=speechSynthesis.getVoices();u.voice=voices.find(v=>/ko-KR/i.test(v.lang))||null;speechSynthesis.speak(u)}
function showAttendanceSuccess(student){const card=document.querySelector(`.student-card[data-student="${student.id}"]`);if(card){card.classList.remove("attendance-success");void card.offsetWidth;card.classList.add("attendance-success");setTimeout(()=>card.classList.remove("attendance-success"),900)}const old=$("attendanceSuccessFlash");old.innerHTML=`<span>✓</span><strong>${escapeHtml(student.name)}</strong><small>출석 완료!</small>`;old.hidden=false;clearTimeout(window.__attendanceSuccess);window.__attendanceSuccess=setTimeout(()=>old.hidden=true,1150);playAttendanceSuccessSound();speakAttendance(`${student.name} 출석 완료!`)}
function setAttendanceVoiceState(listening,label="계명아",help="눌러서 말하기"){state.voice.listening=listening;const button=$("attendanceVoiceRemote");if(!button)return;button.classList.toggle("listening",listening);button.querySelector("strong").textContent=label;button.querySelector("small").textContent=help}
function initAttendanceVoice(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR)return false;if(state.voice.recognition)return true;const r=new SR();r.lang="ko-KR";r.continuous=false;r.interimResults=false;r.maxAlternatives=3;r.onstart=()=>setAttendanceVoiceState(true,"듣고 있어요","이름과 출석을 말해 주세요");r.onend=()=>setAttendanceVoiceState(false);r.onerror=e=>{if(e.error==="not-allowed"||e.error==="service-not-allowed")toast("Chrome 주소창의 마이크 권한을 허용해 주세요.");else if(e.error!=="no-speech"&&e.error!=="aborted")toast(`음성인식 오류: ${e.error}`)};r.onresult=async e=>{const text=clean(e.results?.[0]?.[0]?.transcript).replace(/[.!?。]/g,"");if(!/출석|왔어|왔어요|도착|왔다/.test(text)){toast("예: 김우리 출석이라고 말씀해 주세요.");return}const compact=text.replace(/\s/g,"");let hits=state.students.filter(s=>compact.includes(voiceNameKey(s.name)));if(!hits.length)hits=state.students.filter(s=>voiceAliases(s).some(alias=>compact.includes(voiceNameKey(alias))));if(hits.length!==1){toast(hits.length?"학생 이름을 한 명만 말씀해 주세요.":"학생 이름을 찾지 못했습니다.");return}setAttendanceVoiceState(true,"처리 중","잠시만 기다려 주세요");await markStudent(hits[0],"present")};state.voice.recognition=r;return true}
function toggleAttendanceVoice(){if(!state.session){toast("통합 출석판을 먼저 열어 주세요.");return}if(state.voice.listening){try{state.voice.recognition?.stop()}catch{};return}if(!initAttendanceVoice()){toast("이 브라우저는 음성인식을 지원하지 않습니다.");return}try{state.voice.recognition.start()}catch{toast("마이크를 다시 눌러 주세요.")}}
...
async function loadPeriods(){
  const [pRes,sRes]=await Promise.all([
    ...
    db.from("students").select("id,student_code,name,photo_url,guardians(phone,sms_enabled),enrollments(class_period_id,class_label_raw,training_days,status),kmt_student_voice_aliases(id,alias,alias_key)").order("student_code")
  ]);
